...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const totalStrikeRate = this.innings.reduce(
      (acc, inning) => acc + inning.strikeRate,
      0,
    );
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalEconomy = this.spells.reduce((acc, s) => acc + s.economy, 0);
  ...
  getProfile() {
    // Your code here
    const totalWickets = this.spells.reduce((acc, s) => acc + s.wickets, 0);
