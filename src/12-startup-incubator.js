...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    return new Promise((res, rej) => {
      ...
      if (this.#startups.some((s) => s.startup === startup))
  ...
  removeStartup(name) {
    // Your code here
    const length = this.#startups.length;
    this.#startups = this.#startups.filter((s) => s.startup.name !== name);

    return length !== this.#startups.length;
  }
  ...
  async conductDemo(startupName) {
    ...
    return new Promise((res, rej) => {
      ...
      this.#startups[index].demoCompleted = true;

      res({
        startup: startupName,
        score: Math.floor(Math.random() * 41) + 60,
        feedback: "Bahut badhiya!",
        timestamp: new Date().toISOString(),
      });
    });
  }
